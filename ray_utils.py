...
def resample_along_rays(origins, directions, radii, num_samples, t_vals, weights, randomized, stop_grad, resample_padding, ray_shape):
    """Resampling.
    ...
    if stop_grad:
        with torch.no_grad():
            ...
            new_t_vals = sorted_piecewise_constant_pdf(
                t_vals,
                weights,
                (num_samples + 1) if num_samples is not None else t_vals.shape[-1], #Modified with custom num_samples
                randomized,
            )
    else:
        weights_pad = torch.cat([weights[..., :1], weights, weights[..., -1:]], dim=-1)
